...
getTop5ListPairs = async (req, res) => {
    await Top5List.find({"ownerEmail":req.params.id}, (err, top5Lists) => {//
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        if (!top5Lists) {
...
getTop5ListByUserName = async (req, res) => {
    await Top5List.find({"author":req.params.id}, (err, top5Lists) => {//
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        if (!top5Lists) {
...
getTop5ListByListName = async (req, res) => {
    await Top5List.find({"name":req.params.id}, (err, top5Lists) => {//
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        if (!top5Lists) {
...
getPublishedTop5List = async (req, res) => {
...
module.exports = {
    ...
    getTop5ListByListName,
    getPublishedTop5List
}